...
Shader "Custom/TubeEffect ColorCut"
{    
    Properties
    {
        ...
        _TargetThreshold ("Target Threshold", Range(0.0001, 1.0)) = 0.001
    }
    ...
    HLSLINCLUDE
        ...
        half4 frag(Varyings input) : SV_Target
        {
            ...
            half4 final = half4(YIQ2RGB(yiq * scan), 1);

            // Color cut to alpha
            float3 diff = abs(final.rgb - _TargetColor.rgb);
            float mask = step(_TargetThreshold, dot(diff, float3(1,1,1)));
